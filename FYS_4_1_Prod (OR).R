...
dataLong1 <- dataLong[order(dataLong$Subject, dataLong$Helicopters, dataLong$Language, dataLong$Phase), ]
...
pwc3 <- pwc3 %>% add_xy_position(x = "Phase")
bxp1.1 + stat_pvalue_manual(pwc3, tip.length = 0, hide.ns = TRUE)

pwc4 <- pwc4 %>% add_xy_position(x = "Helicopters")
bxp2.1 + stat_pvalue_manual(pwc4, tip.length = 0, hide.ns = TRUE)
