...
ggline <- ggline(dataLong, x = "Helicopters", y = "Estimation_Accuracy",
                 short.panel.labs = FALSE,
                 add = "mean_se",
                 palette = "npg") +
...
dat <- dataLong$Estimation_Accuracy
...
dataLong %>%
  group_by(Helicopters) %>%
  shapiro_test(Estimation_Accuracy)

ggqqplot(dataLong, "Estimation_Accuracy", ggtheme = theme_bw()) +
