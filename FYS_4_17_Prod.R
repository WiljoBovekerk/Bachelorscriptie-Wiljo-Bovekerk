...
q1 <- quantile(dataLong$Production_Accuracy, 0.25)
q3 <- quantile(dataLong$Production_Accuracy, 0.75)
iqr <- IQR(dataLong$Production_Accuracy)

outliers <- dataLong$Production_Accuracy[dataLong$Production_Accuracy < q1 - 1.5*iqr | dataLong$Production_Accuracy > q3 + 1.5*iqr]
outliers

dataProdno <- dataLong[-c(15,32), ]
...
res.aov1 <- lme(
  Production_Accuracy ~ Helicopters * Phase,          # Fixed effects for main effects and interaction
